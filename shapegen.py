...
def square(im, size=0.2 * Flags.DIM):
    # Generate a square
    vertices = [(Flags.DIM / 2 - size, Flags.DIM / 2 - size), (Flags.DIM / 2 - size, Flags.DIM / 2 + size), (Flags.DIM / 2 + size, Flags.DIM / 2 + size), (Flags.DIM / 2 + size, Flags.DIM / 2 - size)]
...
def simpleCircleDefinedLineCut(im, r=0.3*Flags.DIM):
    ...
    x100Int = (0 - (Flags.DIM / 2) * (math.sqrt(0 - a ** 2 + Flags.DIM * a + r ** 2 - (Flags.DIM / 2)**2) + Flags.DIM / 2) + Flags.DIM / 2 * a + r ** 2) / (a - Flags.DIM / 2)
...
def smarterCut(im):
    ...
    if qr == 0:
        ...
        vertices = [(0, Flags.DIM), (0, y_intercept), (Flags.DIM, randint(0, Flags.DIM - y_intercept)), (Flags.DIM, Flags.DIM)]
